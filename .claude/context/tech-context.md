...
- **Commands**: `scan` (`--sector`), `health`, `universe`, `debate` (`--batch`, `--export md`, `--provider`), `serve`, `outcomes` (collect, summary, backtest, equity-curve), `agency` (ask, chat), `learn` (tune-indicators, tune-votes, status, mine, playbook)
