...
| Symbol | Kind | Signature | Line | Description |
|--------|------|-----------|------|-------------|
| `RecommendedContract` | model | `frozen=True` | 36 | A recommended option contract persisted from a scan run. |
| `ContractOutcome` | model | `frozen=True` | 204 | Outcome data for a recommended contract at a given exit point. |
| `NormalizationStats` | model | `frozen=True` | 294 | Normalization statistics for a single indicator in a scan run. |
| `WinRateResult` | model | `frozen=True` | 357 | Win rate analytics result grouped by signal direction. |
| `ScoreCalibrationBucket` | model | `frozen=True` | 395 | Score calibration analytics result for a score range bucket. |
| `IndicatorAttributionResult` | model | `frozen=True` | 441 | Indicator attribution analytics result. |
| `HoldingPeriodResult` | model | `frozen=True` | 492 | Holding period analytics result. |
| `DeltaPerformanceResult` | model | `frozen=True` | 542 | Delta performance analytics result. |
| `PerformanceSummary` | model | `frozen=True` | 592 | Aggregate performance summary over a lookback period. |
| `AgentAccuracyReport` | model | `frozen=True` | 661 | Per-agent direction accuracy and Brier score. |
| `CalibrationBucket` | model | `frozen=True` | 711 | Single confidence calibration bucket. |
| `AgentCalibrationData` | model | `frozen=True` | 762 | Per-agent or aggregate confidence calibration data. |
| `AgentWeightsComparison` | model | `frozen=True` | 780 | Manual vs auto-tuned weight comparison for a single agent. |
| `EquityCurvePoint` | model | `frozen=True` | 826 | A single data point on the backtesting equity curve. |
| `DrawdownPoint` | model | `frozen=True` | 858 | A single data point on the backtesting drawdown curve. |
| `SectorPerformanceResult` | model | `frozen=True` | 890 | Backtesting performance analytics grouped by GICS sector. |
| `DTEBucketResult` | model | `frozen=True` | 934 | Backtesting performance analytics grouped by days-to-expiration range. |
| `IVRankBucketResult` | model | `frozen=True` | 988 | Backtesting performance analytics grouped by implied volatility rank range. |
| `GreeksDecompositionResult` | model | `frozen=True` | 1042 | Backtesting P&L decomposition by Greeks for a group of contracts. |
| `HoldingPeriodComparison` | model | `frozen=True` | 1078 | Backtesting holding period comparison for a specific duration and direction. |
| `WeightSnapshot` | model | `frozen=True` | 1143 | A point-in-time snapshot of auto-tuned agent weights. |
| `LearningStatus` | model | `frozen=True` | 1207 | Learning system status snapshot for API responses. |
...
| `IntelligenceConfig` | model |  | 394 | Intelligence data configuration — controls yfinance intelligence fetching. |
| `AnalyticsConfig` | model |  | 423 | Analytics persistence configuration — controls outcome collection and batch sizing. |
| `FinancialDatasetsConfig` | model |  | 466 | Financial Datasets AI configuration — controls optional fundamental data enrichment. |
...
| Symbol | Kind | Signature | Line | Description |
|--------|------|-----------|------|-------------|
| `UNLIMITED_SENTINEL` | const | `str` | 3 |  |
...
| `SpreadType` | StrEnum | VERTICAL, IRON_CONDOR, STRADDLE, STRANGLE | 84 | Type of option spread strategy. |
| `MacdSignal` | StrEnum | BULLISH_CROSSOVER, BEARISH_CROSSOVER, NEUTRAL | 93 | MACD crossover signal for market context. |
| `ScanPreset` | StrEnum | FULL, SP500, ETFS, NASDAQ100, RUSSELL2000, MOST_ACTIVE | 106 | Scan universe preset for the scan pipeline. |
| `ScanSource` | StrEnum | MANUAL | 125 | Origin of a scan request. |
| `GreeksSource` | StrEnum | COMPUTED, MARKET, SMOOTHED | 134 | Source of Greeks values on an option contract. |
...
| `OutcomeCollectionMethod` | StrEnum | MARKET, INTRINSIC, EXPIRED_WORTHLESS | 203 | Method used to collect contract outcome data. |
| `GreeksGroupBy` | StrEnum | DIRECTION, SECTOR | 216 | Grouping column for Greeks P&L decomposition. |
| `LLMProvider` | StrEnum | GROQ, ANTHROPIC | 227 | LLM provider for AI debate agents. |
...
| Symbol | Kind | Signature | Line | Description |
|--------|------|-----------|------|-------------|
| `VolSurfaceResult` | class | `(NamedTuple)` | 57 | Result of volatility surface computation. |
| `compute_vol_surface` | func | `(strikes: np.ndarray, ivs: np.ndarray, dtes: np.ndarray, option_types: np.ndarray, spot: float, r...` | 106 | Compute implied volatility surface analytics with tiered fallback. |
| `VolSurfaceIndicators` | class | `(NamedTuple)` | 734 | Indicator signals derived from fitted vol surface. |
| `compute_surface_indicators` | func | `(result: VolSurfaceResult, contract_strike: float, contract_dte: float, strikes: np.ndarray \| Non...` | 755 | Map per-contract z-scores from fitted surface to indicator signals. |
...
| `.fetch_risk_free_rate` | async method | `() -> float` | 177 | Fetch 10-year Treasury yield as a decimal fraction (0.045 = 4.5%). |
| `.fetch_macro_context` | async method | `() -> MacroContext` | 200 | Batch-fetch 8 FRED series and return a ``MacroContext`` snapshot. |
| `.close` | async method | `() -> None` | 218 | Close the httpx client. |
...
| Symbol | Kind | Signature | Line | Description |
|--------|------|-----------|------|-------------|
| `TickerOHLCVResult` | model |  | 56 | Result for a single ticker in a batch OHLCV fetch. |
| `BatchOHLCVResult` | model |  | 72 | Typed result for a batch OHLCV fetch, replacing ``dict[str, list[OHLCV] \| error]``. |
| `BatchQuote` | model | `frozen=True` | 93 | A single ticker's daily price snapshot for heatmap display. |
| `MarketDataService` | class | `(ServiceBase[ServiceConfig])` | 235 | Fetches and normalises yfinance market data into typed Pydantic models. |
| `.fetch_ohlcv` | async method | `(ticker: str, period: str = '1y') -> list[OHLCV]` | 277 | Fetch OHLCV history for *ticker* from yfinance. |
| `.fetch_quote` | async method | `(ticker: str) -> Quote` | 356 | Fetch a real-time quote for *ticker* from yfinance. |
...
| `.get_spread_for_ticker` | async method | `(scan_run_id: int, ticker: str) -> SpreadAnalysis \| None` | 121 | Retrieve the spread analysis for a specific ticker in a scan run. |
...
| Symbol | Kind | Signature | Line | Description |
|--------|------|-----------|------|-------------|
| `strip_think_tags` | func | `(text: str) -> str` | 53 | Remove ``<think>...</think>`` blocks and any stray open/close tags. |
| `PROMPT_RULES_APPENDIX` | const | `str` | 69 |  |
| `build_cleaned_agent_response` | func | `(output: AgentResponse) -> AgentResponse` | 98 | Strip ``<think>`` tags from all text fields of an ``AgentResponse``. |
| `build_cleaned_volatility_thesis` | func | `(output: VolatilityThesis) -> VolatilityThesis` | 124 | Strip ``<think>`` tags from all text fields of a ``VolatilityThesis``. |
| `build_cleaned_flow_thesis` | func | `(output: FlowThesis) -> FlowThesis` | 154 | Strip ``<think>`` tags from all text fields of a ``FlowThesis``. |
| `build_cleaned_contrarian_thesis` | func | `(output: ContrarianThesis) -> ContrarianThesis` | 182 | Strip ``<think>`` tags from all text fields of a ``ContrarianThesis``. |
| `build_cleaned_risk_assessment` | func | `(output: RiskAssessment) -> RiskAssessment` | 208 | Strip ``<think>`` tags from all text fields of a ``RiskAssessment``. |
| `build_cleaned_fundamental_thesis` | func | `(output: FundamentalThesis) -> FundamentalThesis` | 254 | Strip ``<think>`` tags from all text fields of a ``FundamentalThesis``. |
| `build_cleaned_domain_assessment` | func | `(output: T) -> T` | 291 | Strip ``<think>`` tags from all string fields of a ``DomainAssessment`` subclass. |
| `DebateDeps` | dataclass |  | 315 | Injected into every agent via RunContext[DebateDeps]. |
| `DebateResult` | model | `frozen=True` | 337 | Complete debate output returned by run_debate(). |
| `_render_optional` | func | `(label: str, value: float \| None, fmt: str = '.1f') -> str \| None` | 370 | Render a labeled value if non-None and finite, else None. |
| `_render_regime_label` | func | `(label: str, value: float \| None, labels: dict[float, str]) -> str \| None` | 377 | Render a regime field as a human-readable label, with numeric fallback. |
| `_format_dollars` | func | `(value: float) -> str` | 385 | Format a dollar amount as $X.XB or $X.XM, with sign for negatives. |
| `_render_identity_block` | func | `(ctx: MarketContext) -> list[str]` | 397 | Shared identity fields for all domain-specific renderers. |
| `render_trend_context` | func | `(ctx: MarketContext) -> str` | 435 | Render domain-specific context for the Trend agent. |
| `render_volatility_context` | func | `(ctx: MarketContext) -> str` | 478 | Render domain-specific context for the Volatility agent. |
| `render_flow_context` | func | `(ctx: MarketContext) -> str` | 601 | Render domain-specific context for the Flow agent. |
| `render_fundamental_context` | func | `(ctx: MarketContext) -> str` | 648 | Render domain-specific context for the Fundamental agent. |
| `render_macro_context` | func | `(ctx: MarketContext) -> str \| None` | 786 | Render macro-economic context for agent consumption. |
| `_render_neural_context` | func | `(ctx: MarketContext) -> str` | 816 | Render neural trajectory probability context when available. |
| `render_context_block` | func | `(ctx: MarketContext, constraint_warnings: str \| None = None) -> str` | 829 | Render MarketContext as flat key-value text for agent consumption. |
| `compute_citation_density` | func | `(context_block: str, *texts: str) -> float` | 1139 | Compute fraction of context labels referenced in agent output text. |
...
| `compute_correlation_matrix_tool` | async func | `(ctx: RunContext[DeskDeps], ticker: str, comparison_tickers: list[str]) -> str` | 1044 | Compute pairwise correlation matrix between *ticker* and *comparison_tickers*. |
| `compute_risk_adjusted_metrics_tool` | async func | `(ctx: RunContext[DeskDeps], ticker: str) -> str` | 1138 | Compute portfolio-wide risk-adjusted performance metrics. |
| `compute_hv_yang_zhang_tool` | async func | `(ctx: RunContext[DeskDeps], ticker: str, period: int = 20) -> str` | 1212 | Compute Yang-Zhang historical volatility for *ticker*. |
| `compute_garch_forecast_tool` | async func | `(ctx: RunContext[DeskDeps], ticker: str) -> str` | 1284 | Compute a GARCH(1,1) volatility forecast for *ticker*. |
| `compute_markov_regime_tool` | async func | `(ctx: RunContext[DeskDeps], ticker: str) -> str` | 1358 | Detect the current market regime for *ticker* using Markov-switching. |
| `compute_macro_regime_tool` | async func | `(ctx: RunContext[DeskDeps]) -> str` | 1438 | Classify the current macro-economic regime from FRED data. |
| `compute_hurst_exponent_tool` | async func | `(ctx: RunContext[DeskDeps], ticker: str) -> str` | 1494 | Compute the Hurst exponent for *ticker* via rescaled range analysis. |
| `render_available_tools` | func | `(toolset: list[object]) -> str` | 1553 | Generate an ``<<<AVAILABLE_TOOLS>>>`` prompt block from *toolset*. |
| `build_trend_toolset` | func | `() -> list[object]` | 1579 | Return the tools for a Trend Desk agent. |
| `build_flow_toolset` | func | `() -> list[object]` | 1601 | Return the tools for a Flow Desk agent. |
| `build_fundamental_toolset` | func | `() -> list[object]` | 1609 | Return the tools for a Fundamental Desk agent. |
| `build_contrarian_toolset` | func | `() -> list[object]` | 1624 | Return the tools for a Contrarian Desk agent. |
| `build_research_toolset` | func | `() -> list[object]` | 1632 | Return the tools for a Research Desk agent. |
| `synth_fetch_current_quote` | async func | `(ctx: RunContext[object], ticker: str) -> str` | 1681 | Fetch the current quote snapshot for *ticker* from the market context. |
| `synth_fetch_chain_summary` | async func | `(ctx: RunContext[object], ticker: str) -> str` | 1722 | Summarize the available option contracts for *ticker*. |
| `build_synthesis_toolset` | func | `() -> list[object]` | 1809 | Return the tools for the Synthesis agent. |
...
| Symbol | Kind | Signature | Line | Description |
|--------|------|-----------|------|-------------|
| `run_contrarian_desk_query` | async func | `(query: str, deps: DeskDeps, *, model: object \| None = None, config: AgencyConfig \| None = None) ...` | 59 | Run a contrarian desk query with timeout and error handling. |
| `run_contrarian_desk_recommendation` | async func | `(deps: DeskDeps, *, model: Model \| None = None, model_settings: ModelSettings \| None = None, conf...` | 166 | Run the contrarian recommendation agent — never raises. |
...
| Symbol | Kind | Signature | Line | Description |
|--------|------|-----------|------|-------------|
| `run_flow_desk_query` | async func | `(query: str, deps: DeskDeps, *, model: object \| None = None, config: AgencyConfig \| None = None) ...` | 60 | Run a flow desk query with timeout and error handling. |
| `run_flow_desk_recommendation` | async func | `(deps: DeskDeps, *, model: Model \| None = None, model_settings: ModelSettings \| None = None, conf...` | 150 | Run flow desk recommendation -- never raises. |
...
| Symbol | Kind | Signature | Line | Description |
|--------|------|-----------|------|-------------|
| `run_fundamental_desk_query` | async func | `(query: str, deps: DeskDeps, *, model: object \| None = None, config: AgencyConfig \| None = None) ...` | 61 | Run a fundamental desk query with timeout and error handling. |
| `run_fundamental_desk_recommendation` | async func | `(deps: DeskDeps, *, model: Model \| None = None, model_settings: ModelSettings \| None = None, conf...` | 151 | Run fundamental desk recommendation -- never raises. |
...
#### agents/prompts/recommend_contrarian.py

| Symbol | Kind | Signature | Line | Description |
|--------|------|-----------|------|-------------|
| `RECOMMEND_CONTRARIAN_PROMPT` | const |  | 16 |  |

#### agents/prompts/recommend_flow.py

| Symbol | Kind | Signature | Line | Description |
|--------|------|-----------|------|-------------|
| `RECOMMEND_FLOW_PROMPT` | const |  | 16 |  |

#### agents/prompts/recommend_fundamental.py

| Symbol | Kind | Signature | Line | Description |
|--------|------|-----------|------|-------------|
| `RECOMMEND_FUNDAMENTAL_PROMPT` | const |  | 16 |  |

#### agents/prompts/recommend_risk.py
...
| Symbol | Kind | Signature | Line | Description |
|--------|------|-----------|------|-------------|
| `RECOMMEND_TREND_PROMPT` | const |  | 15 |  |

#### agents/prompts/recommend_volatility.py

| Symbol | Kind | Signature | Line | Description |
|--------|------|-----------|------|-------------|
| `RECOMMEND_VOLATILITY_PROMPT` | const |  | 17 |  |
...
| Symbol | Kind | Signature | Line | Description |
|--------|------|-----------|------|-------------|
| `run_trend_desk_query` | async func | `(query: str, deps: DeskDeps, *, model: object \| None = None, config: AgencyConfig \| None = None) ...` | 60 | Run a trend desk query with timeout and error handling. |
| `run_trend_desk_recommendation` | async func | `(deps: DeskDeps, *, model: Model \| None = None, model_settings: ModelSettings \| None = None, conf...` | 150 | Run trend desk recommendation -- never raises. |
...
| Symbol | Kind | Signature | Line | Description |
|--------|------|-----------|------|-------------|
| `run_vol_desk_query` | async func | `(query: str, deps: DeskDeps, *, model: object \| None = None, config: AgencyConfig \| None = None) ...` | 61 | Run a volatility desk query with timeout and error handling. |
| `run_vol_desk_recommendation` | async func | `(deps: DeskDeps, *, model: Model \| None = None, model_settings: ModelSettings \| None = None, conf...` | 151 | Run volatility desk recommendation -- never raises. |
...
| Symbol | Kind | Signature | Line | Description |
|--------|------|-----------|------|-------------|
| `InputShape` | StrEnum |  | 93 | OHLCV column requirements for an indicator function. |
| `IndicatorSpec` | class | `(NamedTuple)` | 107 | Typed registry entry mapping a signal field to an indicator function. |
| `INDICATOR_REGISTRY` | const | `list[IndicatorSpec]` | 131 |  |
| `ohlcv_to_dataframe` | func | `(ohlcv: list[OHLCV]) -> pd.DataFrame` | 155 | Convert OHLCV Pydantic models to a pandas DataFrame for indicators. |
| `compute_indicators` | func | `(df: pd.DataFrame, registry: list[IndicatorSpec]) -> IndicatorSignals` | 184 | Dispatch each registry entry against the DataFrame and populate signals. |
| `compute_options_indicators` | func | `(contracts: list[OptionContract], spot: float) -> IndicatorSignals` | 332 | Compute options-specific indicators from the full option chain. |
| `compute_phase3_indicators` | func | `(contracts: list[OptionContract], spot: float, close_series: pd.Series, dividend_yield: float, ne...` | 432 | Compute DSE indicators that require chain, ticker, or SPX data. |
...
| Symbol | Kind | Signature | Line | Description |
|--------|------|-----------|------|-------------|
| `get_repo` | func | `(request: Request) -> Repository` | 23 | Inject the typed CRUD repository. |
| `get_market_data` | func | `(request: Request) -> MarketDataService` | 28 | Inject the market data service. |
| `get_options_data` | func | `(request: Request) -> OptionsDataService` | 33 | Inject the options data service. |
| `get_fred` | func | `(request: Request) -> FredService` | 38 | Inject the FRED service. |
...
| Symbol | Kind | Signature | Line | Description |
|--------|------|-----------|------|-------------|
| `render_health_table` | func | `(statuses: list[HealthStatus]) -> Table` | 66 | Render health check results as a Rich table. |
| `render_scan_table` | func | `(result: ScanResult) -> Table` | 91 | Render scan results as a Rich table with trading-convention styling. |
| `render_volatility_panel` | func | `(thesis: VolatilityThesis) -> Panel` | 178 | Render Volatility Agent output as a cyan-bordered Rich Panel. |
| `render_flow_panel` | func | `(flow: FlowThesis) -> Panel` | 228 | Render Flow Agent output as a bright_magenta-bordered Rich Panel. |
| `render_fundamental_panel` | func | `(fund: FundamentalThesis) -> Panel` | 270 | Render Fundamental Agent output as a bright_cyan-bordered Rich Panel. |
| `render_risk_panel` | func | `(risk: RiskAssessment) -> Panel` | 317 | Render Risk Agent output as a bright_blue-bordered Rich Panel. |
| `render_contrarian_panel` | func | `(contra: ContrarianThesis) -> Panel` | 369 | Render Contrarian Agent output as a yellow-bordered Rich Panel. |
| `render_spread_panel` | func | `(console: Console, spread: SpreadAnalysis) -> None` | 411 | Render a spread strategy as a Rich table with P&L summary. |
| `render_debate_panels` | func | `(console: Console, result: DebateResult) -> None` | 465 | Render debate result as Rich panels for the 6-agent protocol. |
| `render_batch_summary_table` | func | `(results: list[tuple[str, DebateResult \| None, ...) -> Table` | 670 | Render batch debate results as a compact summary table. |
| `render_debate_history` | func | `(debates: list[DebateRow], ticker: str) -> Table` | 725 | Render past debates as a Rich table. |
...
|------------|--------------|-------|
| `utils/exceptions.py` | `tests/unit/utils/test_exceptions.py` | 11 |
...
| `pricing/neural_surface.py` | `tests/unit/pricing/test_neural_surface.py` | 19 |
| `pricing/spreads.py` | `tests/unit/pricing/test_spreads.py` | 15 |
| `pricing/trajectory.py` | `tests/unit/pricing/test_trajectory.py` | 38 |
...
| `scoring/dimensional.py` | `tests/unit/scoring/test_dimensional.py` | 36 |
...
| `agents/prompts/recommend_contrarian.py` | — | 0 |
| `agents/prompts/recommend_flow.py` | — | 0 |
| `agents/prompts/recommend_fundamental.py` | — | 0 |
| `agents/prompts/recommend_risk.py` | — | 0 |
| `agents/prompts/recommend_trend.py` | — | 0 |
| `agents/prompts/recommend_volatility.py` | — | 0 |
...
| Source File | Test File(s) | Tests |
|------------|--------------|-------|
| `scan/indicators.py` | `tests/unit/scan/test_indicators.py` | 43 |
