...
recommends vol-specific strategies. It analyzes IV rank, IV percentile, term
structure, skew, expected moves, and VIX correlation to determine whether IV
is overpriced, underpriced, or fairly valued. Output is a structured
``VolatilityThesis``.

The dynamic injection of bull/bear arguments (wrapped in ``<<<BULL_ARGUMENT>>>``
and ``<<<BEAR_ARGUMENT>>>`` delimiters) remains in the agent module
``agents/volatility.py`` because it is runtime-dependent, not static prompt text.
"""
