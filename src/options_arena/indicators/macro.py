...
from the ``MacroContext`` snapshot fetched from FRED. Pure computation — no
API calls, no I/O.
...
# Thresholds — MacroContext stores rates as decimal fractions:
...
def compute_macro_regime(macro: MacroContext) -> MacroRegimeResult | None:
    """Classify the macro economic regime from FRED indicators.

    Returns ``None`` when the ``MacroContext`` has fewer than 50% of its
    fields populated (``completeness_ratio() < 0.5``), since the
    classification would be unreliable.

    Args:
        macro: Frozen snapshot of macro-economic indicators from FRED.

    Returns:
        ``MacroRegimeResult`` with regime label, confidence, and contributing
        signals, or ``None`` when data is insufficient.
    """
    if macro.completeness_ratio() < _MIN_COMPLETENESS:
        logger.debug(
            "Macro completeness %.1f%% < %.0f%% threshold, skipping regime classification",
            macro.completeness_ratio() * 100,
            _MIN_COMPLETENESS * 100,
        )
        return None

    yield_spread = macro.yield_spread_10y2y
