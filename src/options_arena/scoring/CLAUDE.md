...
(Import-time sum guard ensures 1.0.)

### Inverted Indicators

Higher raw = worse signal, flipped after normalization: `bb_width`, `atr_pct`, `keltner_width`, `chain_spread_pct`.
`relative_volume` is NOT inverted.

### Composite Score

Direction-agnostic "options interest level". Range [1.0, 100.0]; 0.0 = no data.
Floor value 0.5 in geometric mean (`log(0)` is undefined). Phase 3 recomputes after options indicators populated.
...
DTE: [30, 365]. Liquidity: OI>=100, volume>=1, spread/mid<=30%.
