...
Provides retried fetching with rate limiting and async yfinance wrapping.
Services opt into the helpers they need by calling the protected methods.
No abstract methods — this is a mixin, not an interface.

Generic on ``ConfigT`` so each service can declare its own config type
(``ServiceConfig``, ``FinancialDatasetsConfig``, etc.).
"""
...
class ServiceBase[ConfigT]:
    ...
    Subclasses call the protected ``_retried_fetch`` and ``_yf_call`` helpers
    as needed — none are abstract or required.
