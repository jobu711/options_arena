...
class TestRunTrendDeskRecommendation:
    ...
    async def test_produces_assessment(self) -> None:
        deps = _make_deps()
        result = await run_trend_desk_recommendation(deps, model=TestModel())
    ...
    async def test_no_model_returns_fallback(self) -> None:
        deps = _make_deps()
        result = await run_trend_desk_recommendation(deps)
        assert isinstance(result, TrendAssessment)
    ...
    async def test_timeout_returns_fallback(self) -> None:
        ...
        result = await run_trend_desk_recommendation(
