...
class TestRunVolDeskRecommendation:
    ...
    async def test_produces_assessment(self) -> None:
        deps = _make_deps()
        assessment, usage = await run_vol_desk_recommendation(deps, model=TestModel())
        assert isinstance(assessment, VolatilityAssessment)
        assert assessment.desk == DeskType.VOLATILITY
        assert isinstance(usage, RunUsage)

    async def test_confidence_in_valid_range(self) -> None:
        deps = _make_deps()
        assessment, _usage = await run_vol_desk_recommendation(deps, model=TestModel())
        assert 0.0 <= assessment.confidence <= 1.0

    async def test_no_model_returns_fallback(self) -> None:
        deps = _make_deps()
        assessment, usage = await run_vol_desk_recommendation(deps)
        assert isinstance(assessment, VolatilityAssessment)
    ...
    async def test_timeout_returns_fallback(self) -> None:
        ...
        assessment, usage = await run_vol_desk_recommendation(
            deps,
            model=TestModel(),
            config=config,
        )
    ...
    async def test_fallback_has_correct_ticker(self) -> None:
        ...
        assessment, _usage = await run_vol_desk_recommendation(deps)
    ...
    async def test_fallback_returns_zero_usage(self) -> None:
        ...
        deps = _make_deps()
        _assessment, usage = await run_vol_desk_recommendation(deps)
