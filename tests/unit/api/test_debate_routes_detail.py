"""Tests for agent output fields in API schema and debate routes (#254).

Validates that the API layer correctly surfaces, serializes, and persists
the agent output fields (flow_response, fundamental_response,
risk_response, contrarian_response).
"""
...
def _make_debate_row(debate_id: int = 1) -> DebateRow:
    ...
    return DebateRow(
        ...
        risk_assessment_json=risk.model_dump_json(),
        contrarian_json=contrarian.model_dump_json(),
    )


def _make_debate_row_v1(debate_id: int = 2) -> DebateRow:
    """Create a DebateRow WITHOUT agent output fields (legacy debate)."""
    bull = _make_bull_response()
    thesis = _make_thesis()
...
def _make_debate_result() -> DebateResult:
    """Create a DebateResult with agent output fields populated."""
    bull = _make_bull_response()
    bear = AgentResponse(
        agent_name="bear",
...
# ---------------------------------------------------------------------------
# Test 1: GET /api/debate/{id} includes agent outputs
# ---------------------------------------------------------------------------


async def test_get_debate_returns_agent_fields(
    client: AsyncClient,
    mock_repo: MagicMock,
) -> None:
    ...
    # Agent fields are present and non-None
    ...
    assert data["risk_response"] is not None
    assert data["risk_response"]["risk_level"] == "moderate"
    assert data["risk_response"]["confidence"] == pytest.approx(0.65, abs=0.01)
...
# ---------------------------------------------------------------------------
# Test 2: Legacy debate has None for agent output fields
# ---------------------------------------------------------------------------


async def test_get_debate_legacy_returns_none_agent_fields(
    client: AsyncClient,
    mock_repo: MagicMock,
) -> None:
...
# ---------------------------------------------------------------------------
# Test 4: DebateResultDetail with agent fields serializes to valid JSON
# ---------------------------------------------------------------------------


def test_schema_serialization() -> None:
    """DebateResultDetail with agent fields serializes to valid JSON."""
    flow = _make_flow_thesis()
    fundamental = _make_fundamental_thesis()
    risk = _make_risk_assessment()
    contrarian = _make_contrarian_thesis()
    ...
    # Round-trip check: all agent fields survive serialization
    assert parsed["flow_response"]["direction"] == "bullish"
    assert parsed["fundamental_response"]["catalyst_impact"] == "high"
    assert parsed["risk_response"]["risk_level"] == "moderate"
    assert parsed["contrarian_response"]["dissent_direction"] == "bearish"


# ---------------------------------------------------------------------------
# Test 5: Agent fields are typed Pydantic models, not raw dicts (#258)
# ---------------------------------------------------------------------------


def test_agent_fields_are_typed_models_not_dicts() -> None:
    """Verify agent fields on DebateResultDetail are typed Pydantic models, not dicts."""
    flow = _make_flow_thesis()
    fundamental = _make_fundamental_thesis()
    risk = _make_risk_assessment()
    contrarian = _make_contrarian_thesis()
    ...
    assert detail.risk_response.risk_level == RiskLevel.MODERATE
    assert detail.contrarian_response.dissent_direction == SignalDirection.BEARISH


# ---------------------------------------------------------------------------
# Test 6: GET /api/debate/{id} returns typed agent model JSON (#258)
# ---------------------------------------------------------------------------


async def test_get_debate_returns_typed_agent_model_json(
    client: AsyncClient,
    mock_repo: MagicMock,
) -> None:
    ...
    risk = data["risk_response"]
    assert "risk_level" in risk
    assert "pop_estimate" in risk
    assert "max_loss_estimate" in risk
...
# ---------------------------------------------------------------------------
# Test 7: Legacy debates return null for all agent fields (typed version) (#258)
# ---------------------------------------------------------------------------


async def test_get_debate_null_agent_fields_when_legacy(
    client: AsyncClient,
    mock_repo: MagicMock,
) -> None:
    ...
    # All agent typed model fields should be null
    assert data["flow_response"] is None
    assert data["fundamental_response"] is None
    assert data["risk_response"] is None
    assert data["contrarian_response"] is None
...
# ---------------------------------------------------------------------------
# Test 8: Malformed agent JSON degrades gracefully (#259)
# ---------------------------------------------------------------------------


async def test_malformed_agent_json_degrades_gracefully(
    client: AsyncClient,
    mock_repo: MagicMock,
) -> None:
    """Malformed agent JSON in DB degrades to None (not 500 error)."""
    ...
    assert data["flow_response"] is None
    # Other agent fields still null (no agent data on legacy row)
    assert data["fundamental_response"] is None
    assert data["risk_response"] is None
    assert data["contrarian_response"] is None
...
# ---------------------------------------------------------------------------
# Test 9: Empty string agent JSON treated as absent (#258)
# ---------------------------------------------------------------------------


async def test_empty_string_agent_json_treated_as_none(
    client: AsyncClient,
    mock_repo: MagicMock,
) -> None:
    """Empty string agent JSON is falsy, treated same as None (returns null)."""
    row = _make_debate_row_v1()
...
# ---------------------------------------------------------------------------
# Test 10: DebateResultDetail agent JSON round-trip (#258)
# ---------------------------------------------------------------------------


def test_debate_result_detail_agent_json_roundtrip() -> None:
    """DebateResultDetail with typed agent models survives JSON round-trip."""
    detail = DebateResultDetail(
        id=1,
        ticker="AAPL",
        is_fallback=False,
    ...
    assert isinstance(rebuilt.risk_response, RiskAssessment)
    ...
    assert rebuilt.risk_response.risk_level == RiskLevel.MODERATE
