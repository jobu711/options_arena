...
# ---------------------------------------------------------------------------
# DebateConfig Anthropic fields (#215)
# ---------------------------------------------------------------------------
...
class TestDebateConfigAnthropicFields:
    ...
    def test_thinking_budget_too_low(self) -> None:
        ...
        with pytest.raises(ValidationError, match="thinking_budget_tokens must be in"):
            DebateConfig(thinking_budget_tokens=1023)

    def test_thinking_budget_too_high(self) -> None:
        """thinking_budget_tokens rejects values above 128000."""
        with pytest.raises(ValidationError, match="thinking_budget_tokens must be in"):
            DebateConfig(thinking_budget_tokens=128_001)

    def test_thinking_budget_nan_rejected(self) -> None:
        """thinking_budget_tokens rejects NaN (Pydantic int coercion guard)."""
    ...
    def test_backward_compat_no_args(self) -> None:
        """DebateConfig() with no args still works (existing Groq defaults)."""
        config = DebateConfig()
        assert config.model == "llama-3.3-70b-versatile"
        assert config.provider is LLMProvider.GROQ
        assert config.api_key is None

    def test_model_copy_provider_override(self) -> None:
        """model_copy(update={'provider': ...}) works for CLI override pattern."""
        config = DebateConfig()
        updated = config.model_copy(update={"provider": LLMProvider.ANTHROPIC})
        assert updated.provider is LLMProvider.ANTHROPIC
        assert config.provider is LLMProvider.GROQ  # original unchanged


# ---------------------------------------------------------------------------
# ServiceConfig anthropic_api_key (#215)
# ---------------------------------------------------------------------------


class TestServiceConfigAnthropicKey:
    def test_anthropic_api_key_default_none(self) -> None:
        """Default anthropic_api_key is None."""
        config = ServiceConfig()
        assert config.anthropic_api_key is None

    def test_anthropic_api_key_from_constructor(self) -> None:
        """anthropic_api_key can be set via constructor."""
        config = ServiceConfig(anthropic_api_key="sk-ant-test-123")
        assert config.anthropic_api_key is not None
        assert config.anthropic_api_key.get_secret_value() == "sk-ant-test-123"

    def test_anthropic_api_key_env_override(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """ARENA_SERVICE__ANTHROPIC_API_KEY env var overrides default."""
        monkeypatch.setenv("ARENA_SERVICE__ANTHROPIC_API_KEY", "sk-ant-from-env")
        settings = AppSettings()
        assert settings.service.anthropic_api_key is not None
        assert settings.service.anthropic_api_key.get_secret_value() == "sk-ant-from-env"
