...
import type { IVRankBucketResult } from '@/types'

interface Props {
  data: IVRankBucketResult[]
}
...
const chartData = computed(() => {
  ...
  return {
    labels: props.data.map(d => `${d.iv_min.toFixed(0)}-${d.iv_max.toFixed(0)}%`),
    datasets: [
      {
        label: 'Win Rate %',
...
<template>
  <div class="chart-panel" data-testid="iv-performance-chart">
    <h3>IV Rank Bucket Performance</h3>
    <div v-if="!chartData" class="panel-empty">No IV rank performance data available</div>
